...
export async function listFiles(
  ...
  const [{ total }] = (await db.select({ total: sql`COUNT(*)`.as('total') }).from(schema.files)) || []
...
export async function deleteFile(fileId: string): Promise<void> {
  ...
  const filePath = path.join(config.uploadsDir, fileRow.extension ? `${fileRow.id}.${fileRow.extension}` : fileRow.id)
  await db.delete(schema.files).where(eq(schema.files.id, fileId))
  await fs.unlink(filePath).catch(() => {})
}

export async function updateFile(fileId: string, file: Express.Multer.File): Promise<void> {
  const newUploadPath = path.isAbsolute(file.path) ? file.path : path.join(config.uploadsDir, file.filename)
  ...
  const extension = path.extname(file.originalname).replace('.', '')
  const newPath = path.join(config.uploadsDir, extension ? `${fileId}.${extension}` : fileId)
